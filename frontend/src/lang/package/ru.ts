export default {
  ...
  hysteria: {
    ...
    config: {
      ...
      ech: {
        keyPath:
          "Путь к файлу ключа ECH, созданному с помощью команды `sing-box generate ech-keypair <public_name>`. При запуске сервер выводит в журнал список конфигурации, который необходимо указать клиентам в `tls.ech`. Подробнее см. раздел ECH.",
      },
      obfs: {
        type: "Тип",
        salamander: {
      ...
      bandwidth: {
        ...
        disableLossCompensation:
          "Алгоритм управления перегрузкой Brutal имеет механизм «компенсации потерь»: при потере пакетов он пытается отправлять данные немного быстрее заданной пропускной способности, чтобы компенсировать потери и сохранить целевую скорость. Это может улучшить производительность или, наоборот, ухудшить её в зависимости от условий сети. Установите значение `true`, чтобы отключить компенсацию и всегда отправлять данные строго с заданной скоростью. Обратите внимание, что эта настройка применяется только к текущей конечной точке (она влияет только на направление исходящего трафика этого устройства).",
      },
